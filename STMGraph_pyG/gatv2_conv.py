...
class GATv2Conv(MessagePassing):
    ...
    def __init__(self, in_channels: int,
        ...
        self.lin_l = nn.Parameter(torch.zeros(size=(in_channels, heads * out_channels)))
        nn.init.xavier_normal_(self.lin_l.data, gain=1.414)
        if share_weights:
            self.lin_r = self.lin_l
        else:
            # self.lin_r = Linear(in_channels, heads * out_channels, bias=bias)
            self.lin_r = nn.Parameter(torch.zeros(size=(in_channels, heads * out_channels)))
            nn.init.xavier_normal_(self.lin_r.data, gain=1.414)
    ...
    def forward(self, x: Union[Tensor, PairTensor], edge_index: Adj,
                size: Size = None, return_attention_weights: bool = None,attention=True, tied_attention = None):
        ...
        if isinstance(x, Tensor):
            ...
            if self.share_weights:
                ...
                x_r = torch.mm(x, self.lin_r).view(-1, H, C)
        else:
            x_l, x_r = x[0], x[1]
            assert x[0].dim() == 2
        ...
        # out = self.propagate(edge_index, x=(x_l, x_r),tied_attention=tied_attention,size=size)
        if tied_attention != None:
            # self._alpha = tied_attention
            self.attentions = tied_attention
        else:
            # self._alpha = None
            self.attentions = None
    ...
    def message(self, x_j: Tensor, x_i: Tensor,
        ...
        self._alpha = self.attentions
        alpha = F.dropout(self.attentions, p=self.dropout, training=self.training)
